...
class RainbowLoggingHandler(ColorizingStreamHandler):
 ...
 #           self.reset,
 #           " ",
            self.get_color("cyan", None, False) if self.show_name else "",
        ...
        who = [self.get_color("cyan", None, False),
               getattr(record, "funcName", ""),
               "",
               self.get_color("black", None, False),
               ":",
               self.get_color("cyan", None, False),
        ...
        if len(unformatted_who) < self.who_padding:
            spaces = " " * (self.who_padding + 2 - len(unformatted_who))
        else:
            spaces = ""
