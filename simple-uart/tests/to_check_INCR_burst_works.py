...
import index_to_retrive_data as ir
...
def to_check_INCR_burst_works(dut):
	...
	WID=2
	burst_type="INCR"
	_BRESP = yield axim.write_transaction(address,AxID,up.AxPROT,no_of_beats_in_burst,size_of_beat_in_bytes,burst_type,data,WID)
	...
	size_of_beat_in_bytes = 2
	AxID=3
	d=yield axim.read_transaction(address,AxID,up.AxPROT,no_of_beats_in_burst,size_of_beat_in_bytes,burst_type)
	...
	for x in range(no_of_beats_in_burst):
		if r_data[x]==c_data[x]:
			...
			raise TestFailure("Data read  is incorrect , therefore INCR burst will not work in case of UART slave")
